            ...
            <div class="mb-2">
                <?php foreach ($producto->getCategorias() as $cat): ?>
                    <span class="badge bg-white text-dark me-1"
                        style="border: 1px solid #000;"><?= $cat['nombre']; ?></span>
                <?php endforeach; ?>
            </div>
            <p><?= $producto->getDescripcion(); ?></p>
            <p class="fs-4 fw-bold">Precio: $<?= number_format($producto->getPrecio(), 2, ',', '.'); ?></p>

            <form method="post" action="index.php?sec=carrito" class="mt-4">
                <input type="hidden" name="accion" value="agregar">
                <input type="hidden" name="producto_id" value="<?= intval($producto->getId()) ?>">
                <button type="submit" class="btn btn-success py-3 px-4"><i class="fas fa-plus mx-2"></i>Agregar al
                    carrito</button>
            </form>
